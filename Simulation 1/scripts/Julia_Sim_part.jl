...
function process_file(file, p)
    try
        ...
        # problem with multithreading and performance?
    catch e
...
# Get list of files
files = filter(f -> occursin(r"^data_5000_\d{1,3}\.arrow$", f), readdir("Simulation 1/datasets/"))
#files = readdir("Simulation 1/datasets/")
#files_comp = readdir("Simulation 1/out/Julia/")
#files = setdiff(files, files_comp)
