...
log_file <- "out/R/error_log.txt"

#tic()
files <- data.frame(file = list.files("out/datasets/"))

est <- function(file) {
  tryCatch({
    data <- read_feather(paste0("out/datasets/", file))
    ...
    write_feather(out_surv, sink = paste0("out/R/R_MRD_", file))
  }, error = function(e) {
